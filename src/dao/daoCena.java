...
public class daoCena {
    ...
    public String sql_insert="INSERT INTO cenas (`comedor_id`, `expediente_id`, `created_at`) values(?,?,?);";
    ...
    "inner join facultads f inner join cenas d inner join  comedors c on s.user_id=u.id and d.comedor_id=c.id " +
   ...
   public boolean Editar(Cena c,Object key) {
     PreparedStatement ps;
        try {            
            ps=cn.getCon().prepareStatement(sql_insert);
    ...
    public List<Cena> Mostrar() {
      List<Cena> listar=new ArrayList<>();
        try {            
            Cena d;ResultSet r;
